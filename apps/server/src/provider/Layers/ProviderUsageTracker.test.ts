...
function buildTrackerHarness() {
  return Effect.gen(function* () {
    ...
    const publishedSnapshotsRef = yield* Ref.make<Array<unknown>>([]);

    yield* tracker.start().pipe(Effect.provide(providerServiceLayer));
    yield* Stream.runForEach(tracker.streamChanges, (snapshot) =>
      Ref.update(publishedSnapshotsRef, (snapshots) => [...snapshots, snapshot]),
    ).pipe(Effect.forkScoped);

    return {
      tracker,
      publish: (event: ProviderRuntimeEvent) => PubSub.publish(runtimeEvents, event),
      readPublishedSnapshots: Ref.get(publishedSnapshotsRef),
    } as const;
  });
}
...
describe("ProviderUsageTracker", () => {
    ...
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          assert.deepStrictEqual(yield* harness.readPublishedSnapshots, []);
        }),
      ),
    );
  });
    ...
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const publishedSnapshots = yield* harness.readPublishedSnapshots;
  ...
  it("ignores unsupported provider rate-limit events", async () => {
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          const harness = yield* buildTrackerHarness();
